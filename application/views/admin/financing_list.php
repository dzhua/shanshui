						...
						<div class="data-container">
							<header>
								...
								<ul class="data-header-actions">
									<li class="demoTabs"><a href="/admin/financing/create" class="btn">创建新项目</a></li>
								</ul>
							</header>
								...
								<div id="static" class="tab-pane active">
								  ...
								  <div class="span8">
									...
									<table class="table table-striped table-hover">
										...
										<tbody>
											<?php foreach ($financing_list as $financing) { ?>
											<tr>
												<td><a href="#" target="_blank"><?php echo $financing['id'];?></a></td>
												<!-- <td><img src="<?php echo get_thumb_pic($financing['img_name'], 'm'); ?>" style="height:100px"/></td> -->
												...
												<td><?php echo $financing['status'];?></td>
												<td class="toolbar">
													<div class="btn-group">
														<button tag="<?php echo $financing['id'];?>" class="btn upd"><span class="awe-wrench"></span></button>
														<button tag="<?php echo $financing['id'];?>" class="btn del"><span class="awe-remove"></span></button>
													</div>
												</td>
											</tr>
											<? } ?>
...
<script>
	$(".page").click(function(){
		...
		var lock_start = check($("#lock_start").val());
		var lock_stop = check($("#lock_stop").val());
		var status = check($("#status").val());
		location.href = '/admin/financing/'+company_name+'_'+item_name+'_'+buy_start+'_'+buy_stop+'_'+lock_start+'_'+lock_stop+'_'+status+"/"+$(this).attr('tag');
		return false;
	});
	$(".del").click(function(){
	...
	$("#act_search").click(function(){
		...
		var lock_stop = check($("#lock_stop").val());
		var status = check($("#status").val());
		location.href = '/admin/financing/'+company_name+'_'+item_name+'_'+buy_start+'_'+buy_stop+'_'+lock_start+'_'+lock_stop+'_'+status;
	});
	$(".upd").click(function(){
		var id = $(this).attr("tag");
