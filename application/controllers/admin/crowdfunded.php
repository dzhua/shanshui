...
class Crowdfunded extends CI_Controller {
...
// 			redirect('/admin/login');
	...
	public function create() {
		...
		$data['main_content'] = $this->load->view('admin/crowdfunded_create', $data, TRUE);
	...
	public function do_create() {
		...
		if ($_FILES['userfile']['name']) {	
			...
			$file_name	= time().'.jpg';
			$file_path 	= APP_SCRIPT.IMG_CROWDFUNDED_UPLOAD_PATH;
			...
			if(move_uploaded_file($tmp_name, $file_path.$file_name)){
				...
				if( file_exists($path) ){
					...
					$crowdfunded['cover_path'] = IMG_CROWDFUNDED_UPLOAD_PATH . $file_name;
				}
			}
		}
		...
		$crowdfunded_id = $this->common_model->ins_data('crowdfunded', $crowdfunded);
		...
		if ($_FILES['userfile']['name']) {	
			...
			if(move_uploaded_file($tmp_name, $file_path.$file_name)){
				...
				if( file_exists($path) ){
					...
					$crowdfunded['img_name'] = IMG_CROWDFUNDED_UPLOAD_PATH . $file_name;
				}
			}
		}
