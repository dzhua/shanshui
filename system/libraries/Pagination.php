...
class CI_Pagination {
	...
	function create_links()
	{
		...
		if($this->cur_page>$this->num_links){
			...
			$end   = ((2*$this->num_links + 1) > $num_pages) ? $num_pages : (2*$this->num_links+1);
		}
		...
		if  ($this->cur_page != 1)
		{
			...
			$i = $uri_page_number-1;//为了页数和URL一至
			if ($i == 0 || $i == 1) 
			{
				$link = substr(trim($this->base_url),0,strlen(trim($this->base_url))-1);
			}else
			{
				$link = $this->base_url.$i;
			}
			$output .= $this->prev_tag_open.'<a href="'.$link.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
		}
		...
		for ($loop = $start; $loop <= $end; $loop++)
		{
			...
			if ($loop >= 0)
			{
				...
				else
				{
					...
					{
						$link = substr(trim($this->base_url),0,strlen(trim($this->base_url))-1);
						$output .= $this->num_tag_open.'<a href="'.$link.'">'.$loop.'</a>'.$this->num_tag_close;
					}
					else
		...
		if (($this->cur_page + (2*$this->num_links)) < $num_pages)
		{
			//$i = (($num_pages * $this->per_page) - $this->per_page);
			$i=$num_pages;//为了页数和URL一至
